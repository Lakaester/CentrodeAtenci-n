/**
 * Tipos de la integración Localbi (Historia Clínica de Restaurant.pe).
 * Reflejan EXACTAMENTE el contrato documentado en
 * GUIA-INTEGRACION-FRONTEND-HISTORIA-CLINICA.md.
 * No se recalcula ni se transforma ningún valor.
 */

export type LocalbiTipo = "1" | "2" | "3";

export interface LocalbiEnvelope<T = unknown> {
  tipo: LocalbiTipo | string;
  mensajes: string[];
  data: T;
  logs: unknown[];
...
export interface LocalbiBusquedaResponse {
  tipo: LocalbiTipo | string;
  mensajes: string[];
  totalRegistros: number;
  data: LocalbiUnidadNegocio[];
  logs: unknown[];
}
