...
export class AcceptTicketUseCase {
  constructor(private repo: ITicketRepository) {}
  async execute(id: string, asesorId: string, asesorNombre: string): Promise<TicketLifecycleResult> {
    const ticket = await this.repo.findById(id);
    if (!ticket) return { success: false, error: "Ticket no encontrado" };
    const result = TicketLifecycle.accept(ticket, asesorId, asesorNombre);
