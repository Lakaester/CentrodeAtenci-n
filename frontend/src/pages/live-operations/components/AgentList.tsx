...
export function AgentList({ items, state, error, onRetry, className }: Props) {
  ...
  if (state === "empty" || items.length === 0) {
    return (
      <div className={cn("flex items-center justify-center p-4 text-center", className)}>
        <p className="text-xs text-black-25">Sin agentes disponibles</p>
      </div>
    );
  }

  return (
    <div className={cn("space-y-2", className)} role="list" aria-label="Asesores disponibles">
      {items.map((item) => (
